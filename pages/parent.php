...
?>
 <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header"> <i class="fa fa-group fa-fw"></i>Parent</h1>
            </div>
            <!-- /.col-lg-12 -->
            <?php 
                if(isset($_GET['msg'])){
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                      <div class="col-lg-3">
                        ...
                        <form action="parent-process.php" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
                            <div class="img-box">
                                  ...
                                  if (mysqli_num_rows($result) > 0) {                                     
                                      ...
                                      while($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        if($row['gender']==1){
                                          ...
                                          echo '<tr onclick="javascript:showParentModal(this);">';
                                           echo '<td>'.$row['aid'].'</td>';
    ...
    <div class="modal fade" id="parentModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabelParent" aria-hidden="true" >
            <div class="modal-dialog" style="width:1080px;">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h4 class="modal-title" id="myModalLabelParent">Parent Detail <span id="liquidationDate" style="color:orange;"></span></h4>                                                
                    </div>

                    <form id="myform" action="parent-process.php" method="post" enctype="multipart/form-data" >
                       
                        <div class="modal-body parent-modal-body">
  ...
  <script language="javascript" type="text/javascript">
           ...
           function showParentModal(row){

                var x=row.cells;
                 var id = x[0].innerHTML;
                
                $('.parent-modal-body').load('parent-modal.php?id=' + id,function(){
                     
                    $('#parentModal').modal({show:true}); 
            ...
            function validateForm(){
              ...
              document.getElementById('password2').value = pword;
            ...
            function addStudent(row){
               ...
               var firstname =x[1].innerHTML;
               var middlename =x[2].innerHTML;
               var lastname =x[3].innerHTML;
               var classsection=x[4].innerHTML;
               var name = firstname+' '+middlename+' '+lastname;
               document.getElementById('popstudent-id').value = studentid;
               document.getElementById('student-name').value = name; 

                document.getElementById("btnModalSaveStudent").disabled = false;
            }
            function saveChildToParent(){

              var childid=document.getElementById('popstudent-id').value;
              var parentid = document.getElementById("selectedId").value;
               $.ajax({
                type: 'post',
                url: 'ajaxparentchild.php',
                data: {
                 childid:childid, 
                 parentid:parentid,      
                },
                success: function (response) {
                 ...
                 $( '#showchild' ).html(response);
                }
                });
              
            }
            function showConfirmRemove(row){
               var x=row.cells;
               var childid =x[0].innerHTML;
               document.getElementById('confirmChildId').value = childid;

            }
            function removeChildFromList(){
              var action = "remove";
               var parentid = document.getElementById("selectedId").value;
              var confirmfamilyId = document.getElementById('confirmChildId').value;
               $.ajax({
                type: 'post',
                url: 'ajaxparentchild.php',
                data: {
                 action:action, 
                 confirmfamilyId:confirmfamilyId,    
                 parentid:parentid,  
                },
                success: function (response) {
                 // We get the element having id of display_info and put the response inside it
